...
def _parse_wire_sections_txt(txt_path: str):
    """Parses the legacy Wire_Sections.txt file."""
    unit_str = None
    ...
    with open(txt_path, "r", encoding="utf-8", errors="ignore") as f:
        for raw in f:
            ...
            if line:
                unit_str = line
                break
        if unit_str is None: raise ValueError("Missing unit line.")
        scale = _unit_scale(unit_str)

        for raw in f:
            if raw.strip().startswith("Section-"):
                _add_vertex_from_txt(raw.strip(), sections, scale)
                break
        for raw in f:
            if raw.strip().startswith("Section-"):
                _add_vertex_from_txt(raw.strip(), sections, scale)
...
def _add_vertex_from_txt(line: str, sections: Dict[str, SectionData], scale: float):
    parts = [p.strip() for p in line.split(",")]
    if len(parts) < 4: return
    sec_name = parts[0]
    try:
        x_mm, y_mm, z_mm = float(parts[1]) * scale, float(parts[2]) * scale, float(parts[3]) * scale
    except Exception:
        return
...
def _parse_geometry_json(json_path: str):
    """Parses the new geometry.json file."""
    with open(json_path, 'r', encoding='utf-8') as f:
        data = json.load(f)

    unit_str = data.get("units", "mm")
    scale = _unit_scale(unit_str)
    sections_data = data.get("sections", [])
    if not sections_data: raise ValueError("No sections found in geometry.json")
    ...
    for sec_data in sections_data:
        ...
        z_mm = sec_data.get("z_level", 0.0)
        section = SectionData(z_mm=z_mm)
        section.points = [(v.get('x', 0.0), v.get('y', 0.0)) for v in sec_data.get("vertices", [])]
        sections[sec_name] = section

    return unit_str, scale, sections

def parse_geometry_file(folder: str) -> Tuple[str, float, Dict[str, SectionData]]:
    """Dispatcher: Parses geometry from either geometry.json or Wire_Sections.txt."""
    folder_path = Path(folder)
    json_path = folder_path / "geometry.json"
    txt_path = folder_path / "Wire_Sections.txt"

    if json_path.exists():
        return _parse_geometry_json(str(json_path))
    elif txt_path.exists():
        return _parse_wire_sections_txt(str(txt_path))
    else:
        raise FileNotFoundError(f"No geometry file found in {folder}")
...
class ImportWireSectionsPlugin(pcbnew.ActionPlugin):
    def defaults(self):
        ...
        self.description = "Read spirals.json → find geometry.json (or Wire_Sections.txt) → draw tracks per Section; map Z to layers and set copper layer count automatically."
    ...
    def Run(self):
        ...
        parent = None
        spirals_json_path = ask_file_open(parent, "Choose spirals.json", "JSON files (*.json)|*.json|All files|*.*")
        if not spirals_json_path:
            return

        try:
            from pathlib import Path
            with open(spirals_json_path, 'r', encoding='utf-8') as f: data = json.load(f)
            spirals = data.get("spirals", [])
            if not isinstance(spirals, list): raise ValueError("'spirals' key does not contain a list.")
            ...
            for s in spirals:
                ...
                if p_str:
                    p = Path(p_str)
                    if not p.is_absolute(): p = Path(spirals_json_path).parent / p
                    folders.append(str(p.resolve()))
        except Exception as e:
            wx.MessageBox(f"Failed to read or parse spirals.json:\n{e}", "Import Wire Sections", wx.OK | wx.ICON_ERROR)
            return

        if not folders:
            wx.MessageBox("spirals.json contains no valid folder paths.", "Import Wire Sections", wx.OK | wx.ICON_WARNING)
            return

        mode = ask_mode(parent)
        if mode is None: return
        width_mm = ask_track_width(parent, default_mm=0.25)
        if width_mm is None: return
        fixed_layer = None
        if mode == "single":
            fixed_layer = ask_layer(parent, default="F.Cu")
            if fixed_layer is None: return

        save_copies = ask_save_copies(parent)
        total_segments, summary_lines, saved_paths = 0, [], []

        for folder in folders:
            try:
                unit_str, _, sections = parse_geometry_file(folder)
                segs, per_layer, created_segments = draw_sections(board, sections, mode, width_mm, fixed_layer, True)
                total_segments += segs
                layer_stats = ", ".join([f"{k}:{v}" for k, v in sorted(per_layer.items())])
                summary_lines.append(f"✓ {os.path.basename(folder)} → Segments:{segs} (layers:{layer_stats})")
                ...
                # Clean up the added tracks for the next iteration
                for seg in created_segments:
                    board.Remove(seg)

                # We need to refresh the view after removing tracks
                try: pcbnew.Refresh()
                except Exception: pass

            except Exception as e:
                summary_lines.append(f"❌ {os.path.basename(folder)} — ERROR: {e}")
                # Also try to clean up any partially added tracks on error
                if 'created_segments' in locals():
                    for seg in created_segments:
                        board.Remove(seg)
                continue

...
def _parse_wire_sections_txt(path: Path) -> Tuple[str, str | None, OrderedDict[str, SectionPoints]]:
    """Parse the legacy FreeCAD ``Wire_Sections.txt`` file."""
...
def _parse_geometry_json(path_to_json: Path) -> Tuple[str, str | None, OrderedDict[str, SectionPoints]]:
    ...
    for sec_data in sections_data:
        ...
        z_level = sec_data.get("z_level", 0.0)
        vertices = sec_data.get("vertices", [])
        
        points: SectionPoints = []
        for v in vertices:
            points.append((v.get('x', 0.0), v.get('y', 0.0), z_level))
        
        sections[sec_name] = points

    return units, None, sections

def parse_wire_sections(path: Path) -> Tuple[str, str | None, OrderedDict[str, SectionPoints]]:
    """
    Dispatcher function. Checks for geometry.json and calls the appropriate parser.
    ...
    elif wire_sections_txt_path.exists():
        return _parse_wire_sections_txt(wire_sections_txt_path)
    elif path.is_file() and path.name == "Wire_Sections.txt": # Final fallback
         return _parse_wire_sections_txt(path)
    else:
        raise FileNotFoundError(f"No geometry file (geometry.json or Wire_Sections.txt) found in {directory}")

...
def write_spirals_json(
    mother: Path,
    created_folders: List[Path],
    all_combos: List[Tuple[Any, ...]],
    nfmt: str,
    filename: str = "spirals.json"
) -> None:
    """
    Writes a JSON file containing the details of all generated spiral variants.
    """
    spirals_data = []

    # Create a lookup of folder names to their combo data
    # The combo is a tuple of tuples, so we need to convert it to a list of lists/tuples
    combo_map = {
        make_combo_folder_name(list(combo), nfmt): combo for combo in all_combos
    }

    for path in created_folders:
        folder_name = path.name
        combo_data = combo_map.get(folder_name)

        metadata = {}
        if combo_data:
            # combo_data is a tuple of (K, N, dir) tuples
            metadata["layers"] = len(combo_data)
            metadata["per_layer"] = [
                {"K": k, "N": n, "dir": d} for k, n, d in combo_data
            ]
            # Sum of K can be useful
            metadata["total_K"] = sum(k for k, _, _ in combo_data)
    ...
    project_name = mother.name
    creation_date = datetime.datetime.now(datetime.timezone.utc).isoformat()

    output_data = {
        "project_name": project_name,
        "creation_date": creation_date,
        "spirals": spirals_data,
    }

    json_path = mother / filename
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(output_data, f, indent=2)


def verify_spirals_json(mother: Path, filename: str = "spirals.json") -> tuple[bool, str]:
    """
    Quick sanity check that spirals.json exists, is valid JSON, and all paths inside exist.
    """
    json_path = mother / filename
    ...
    if "spirals" not in data or not isinstance(data["spirals"], list):
        return False, f"{filename} is missing the 'spirals' list."

    entries = data["spirals"]
    if not entries:
        return True, f"{filename} is present but contains no spiral entries."

    missing_paths = [p for entry in entries if not (p := Path(entry.get("path", ""))).exists()]

    if missing_paths:
        msg = f"{filename} is valid, but {len(missing_paths)} path(s) do not exist:\n" + "\n".join(map(str, missing_paths))
        return False, msg

    return True, f"{filename} is valid and contains {len(entries)} entries."
...
class BatchApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Sampling & header
        lf_sim = ttk.LabelFrame(tab_geom, text="Sampling")
    ...
    def on_generate(self):
        ...
        write_json = getattr(self.SDU, "write_geometry_json")
        ...
        for per_layer in combos_per_layer:
            ...
            if outdir.exists():
                ...
                json_path = outdir / "geometry.json"
                ...
                try:
                    ...
                    self._log(f"OK  → {subname}/geometry.json  (Sections={len(arms_xy)})")
                except Exception as exc:
                    skipped += 1
        ...
        # spirals.json in mother folder
        write_spirals_json(mother, outdirs, combos_per_layer, nfmt)
        ok, msg = verify_spirals_json(mother)
...
def main():
    ...
    required = ["SpiralInputs", "build_multiarm_geometry", "write_geometry_json"]

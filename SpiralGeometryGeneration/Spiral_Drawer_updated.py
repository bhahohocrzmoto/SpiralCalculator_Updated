...
def write_geometry_json(
    ...
    """
    Writes a geometry.json file containing the structured representation of the spiral.
    """
    json_path = Path(json_path_str)
    sections_data = []
    for i, (poly, z) in enumerate(zip(arms_xy, z_levels_mm), start=1):
        section = {
            "name": f"Section-{i}",
            "z_level": z,
            "vertices": [{"x": round(x, 8), "y": round(y, 8)} for x, y in poly]
        }
...
def write_simple_dxf_lwpolylines(json_path_str: str, dxf_name: str):
    """
    Calls the converter script to create a DXF from the geometry.json file.
    """
    workdir = Path(__file__).resolve().parent
    in_json = Path(json_path_str).resolve()
    out_dxf = in_json.parent / dxf_name

    # The converter script is in a sibling directory
    conv = workdir / "Optional TXT to DXF" / "json2dxf_sections.py"
    if not conv.exists():
    ...
    res = subprocess.run(
        [sys.executable, str(conv), str(in_json)],
        cwd=in_json.parent, text=True, capture_output=True
    )
    if res.returncode != 0:
        raise RuntimeError(
            "json-to-dxf conversion failed "
            f"(exit {res.returncode}).\nSTDOUT:\n{res.stdout}\nSTDERR:\n{res.stderr}"
        )

    produced = in_json.with_suffix(".dxf")
...
class SpiralApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.var_json_path = tk.StringVar(value="geometry.json")
    ...
    def _build_layout(self):
        ...
        self._add_labeled_entry(f, "JSON path:", self.var_json_path, row, width=36); row+=1
        ...
        ttk.Button(btnbar, text="Save (DXF + JSON)", command=self.on_save).grid(row=0, column=1, padx=8, pady=6, sticky="e")
    ...
    def on_save(self):
        """
        Save DXF + JSON using current geometry (rebuild if needed).
        """
        ...
        json_path = self.var_json_path.get().strip() or "geometry.json"
        ...
        try:
            # Always write JSON first so the converter has an input.
            write_geometry_json(
                arms_xy,
                zs,
                json_path,
            )
            # Now convert JSON → DXF (with robust checks inside)
            write_simple_dxf_lwpolylines(json_path, dxf_path)
        except Exception as e:
